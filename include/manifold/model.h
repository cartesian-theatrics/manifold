...
class Model {
 public:
  ...
  MeshGL GetMeshGL(int normalIdx = -1) const {
    return geometry_.GetMeshGL(normalIdx);
  }
  MeshGL64 GetMeshGL64(int normalIdx = -1) const {
    return geometry_.GetMeshGL64(normalIdx);
  }
  const std::map<uint32_t, ModelAppearance>& Surfaces() const {
    return surfaces_;
  }
  size_t LayerCount() const {
    std::set<uint64_t> ids;
    for (const auto& s : surfaces_)
      for (const auto& l : s.second.layers) ids.insert(l.id);
    return ids.size();
  }
  ...
  size_t ImageCount() const {
    ...
    for (const auto& s : surfaces_)
      for (const auto& l : s.second.layers) images.insert(l.image.get());
    return images.size();
  }
  ...
  Model Texture(std::shared_ptr<const ModelImage> image, const Mapping& mapping,
                ...
                bool repeat = false,
                std::array<double, 4> coverage = {0, 0, 1, 1}) const {
    if (!image || !mapping || !std::isfinite(opacity) || opacity < 0 ||
        opacity > 1)
      throw std::invalid_argument(
          "Texture requires an image, mapping and opacity in [0,1]");
    for (double x : coverage)
      if (!std::isfinite(x)) throw std::invalid_argument("Invalid UV coverage");
    if (coverage[0] >= coverage[2] || coverage[1] >= coverage[3])
      throw std::invalid_argument("Invalid UV coverage");
    int index = NextProperty();
    Model result = Keep(mapping(geometry_, index));
    if (result.NextProperty() < index + 2)
      throw std::invalid_argument("Mapping did not append UV channels");
    ModelLayer layer{NextLayerID(), name,   std::move(image), index,
                     opacity,       repeat, coverage};
    for (auto& surface : result.surfaces_)
      surface.second.layers.push_back(layer);
  ...
  Model Color(vec4 rgba) const {
    for (int i = 0; i < 4; ++i)
      if (!std::isfinite(rgba[i]) || rgba[i] < 0 || rgba[i] > 1)
        throw std::invalid_argument(
            "Color must be finite linear RGBA in [0,1]");
    Model result = *this;
    for (auto& surface : result.surfaces_) {
  ...
  Model operator^(const Model& b) const {
    return Boolean(b, OpType::Intersect);
  }
  ...
  Model Rotate(double x, double y = 0, double z = 0) const {
    return Keep(geometry_.Rotate(x, y, z));
  }
  Model Mirror(vec3 v) const { return Keep(geometry_.Mirror(v)); }
  Model Transform(const mat3x4& m) const {
    return Keep(geometry_.Transform(m));
  }
  Model Warp(std::function<void(vec3&)> f) const {
    return Keep(geometry_.Warp(f));
  }
  Model Refine(int n) const { return Keep(geometry_.Refine(n)); }
  Model RefineToLength(double n) const {
    return Keep(geometry_.RefineToLength(n));
  }
  Model RefineToTolerance(double n) const {
    return Keep(geometry_.RefineToTolerance(n));
  }
  Model SmoothOut(double angle = 60, double smoothness = 0) const {
    return Keep(geometry_.SmoothOut(angle, smoothness));
  }
  Model Simplify(double n) const { return Keep(geometry_.Simplify(n)); }
  ...
  Model CalculateNormals(int index, double angle = 60) const {
    if (index + 3 < NextProperty())
      throw std::invalid_argument("Normals must append after model properties");
  ...
  std::pair<Model, Model> Split(const Model& cutter) const {
    return {*this ^ cutter, *this - cutter};
  }
  ...
  double MinGap(const Model& b, double length) const {
    return geometry_.MinGap(b.geometry_, length);
  }
  ...
  vec4 SampleColor(size_t triangle, double b1, double b2) const {
    if (!std::isfinite(b1) || !std::isfinite(b2) || b1 < 0 || b2 < 0 ||
        b1 + b2 > 1)
  ...
  ModelBake Bake(int tileSize = 16) const {
    if (tileSize < 2 || tileSize > 256)
      throw std::invalid_argument("Atlas tile size must be 2..256");
    ...
    int columns = int(std::ceil(std::sqrt(double(count)))),
        stride = tileSize + 2 * gutter;
    int rows = int((count + columns - 1) / columns);
    if (uint64_t(columns) * rows * stride * stride > 67108864)
      throw std::invalid_argument(
          "Atlas exceeds 64 million pixels; reduce :tile-size or model "
          "complexity");
    ModelBake result;
    result.width = columns * stride;
    result.height = rows * stride;
    ...
    for (size_t i = 0; i < input.mergeFromVert.size(); ++i)
      parent[root(input.mergeFromVert[i])] = root(input.mergeToVert[i]);
    auto position = [&](size_t face, int corner) {
      const double* p =
          &input.vertProperties[input.triVerts[3 * face + corner] *
                                input.numProp];
    ...
    for (size_t f = 0; f < count; ++f)
      for (int c = 0; c < 3; ++c)
        lengths[f] = std::max(
            lengths[f], la::length(position(f, c) - position(f, (c + 1) % 3)));
    std::nth_element(lengths.begin(), lengths.begin() + count / 2,
                     lengths.end());
    ...
    for (size_t face = 0; face < count; ++face) {
      ...
      for (int c = 1; c < 3; ++c)
        if (la::length(position(face, c) - position(face, (c + 1) % 3)) >
            la::length(position(face, a) - position(face, (a + 1) % 3)))
          a = c;
      int b = (a + 1) % 3, c = (a + 2) % 3;
      vec3 edge = position(face, b) - position(face, a),
           other = position(face, c) - position(face, a);
      double length = la::length(edge),
             density = (tileSize - 1) / std::max(length, typicalLength);
      double x = la::dot(other, edge) / length;
      double height = la::length(la::cross(edge, other)) / length;
      std::array<vec2, 3> chart;
      chart[a] = {0, 0};
      chart[b] = {length * density, 0};
      chart[c] = {x * density, height * density};
      auto barycentric = [&](vec2 point) {
        double v = point.y / chart[c].y,
               u = (point.x - v * chart[c].x) / chart[b].x;
        ...
        if (u >= 0 && v >= 0 && u + v <= 1) {
          ...
          for (int i = 0; i < 3; ++i) {
            int j = (i + 1) % 3;
            vec2 d = chart[j] - chart[i];
            double squared = la::dot(d, d);
            double t = squared == 0
                           ? 0
                           : std::clamp(la::dot(point - chart[i], d) / squared,
                                        0.0, 1.0);
            vec2 delta = point - (chart[i] + t * d);
            double candidate = la::dot(delta, delta);
      ...
      for (int y = 0; y < stride; ++y)
        for (int x = 0; x < stride; ++x) {
          ...
          if (color.x != previous.x || color.y != previous.y ||
              color.z != previous.z || color.w != previous.w) {
            for (int c = 0; c < 3; ++c) encoded[c] = Encode(color[c]);
            encoded[3] =
                uint8_t(std::lround(std::clamp(color.w, 0.0, 1.0) * 255));
            previous = color;
          }
          std::copy(encoded.begin(), encoded.end(),
                    result.rgba.begin() + offset);
        }
      for (int corner = 0; corner < 3; ++corner) {
        uint64_t vertex = input.triVerts[3 * face + corner],
                 physical = root(vertex);
        uint32_t index = uint32_t(result.mesh.NumVert());
        const double* p = &input.vertProperties[vertex * input.numProp];
        result.mesh.vertProperties.insert(
            result.mesh.vertProperties.end(),
            {float(p[0]), float(p[1]), float(p[2]),
             float((tx + gutter + 0.5 + chart[corner].x) / result.width),
             float((ty + gutter + 0.5 + chart[corner].y) / result.height),
             float(p[normalIndex + 3]), float(p[normalIndex + 4]),
             float(p[normalIndex + 5])});
        result.mesh.triVerts.push_back(index);
        if (first[physical] == UINT32_MAX)
          first[physical] = index;
        else {
          result.mesh.mergeFromVert.push_back(index);
          result.mesh.mergeToVert.push_back(first[physical]);
        }
      }
    }
    result.mesh.runIndex.assign(input.runIndex.begin(), input.runIndex.end());
    result.mesh.runOriginalID = input.runOriginalID;
    result.mesh.runTransform.assign(input.runTransform.begin(),
                                    input.runTransform.end());
 ...
 private:
  ...
  static void CheckGeometry(const Manifold& geometry) {
    if (geometry.Status() != Manifold::Error::NoError)
      throw std::invalid_argument("Operation produced invalid Model geometry");
  }
  void Prune() {
    ...
    for (size_t run = 0; run < mesh.runOriginalID.size(); ++run) {
      ...
      if (!surfaces_.count(id))
        throw std::invalid_argument(
            "Operation lost surface appearance provenance");
      live.insert(id);
    }
  ...
  Model Reidentify() const {
    ...
    for (auto& id : mesh.runOriginalID) {
      ...
      if (found == ids.end()) {
        ...
        if (surfaces_.count(id))
          result.surfaces_.emplace(replacement, surfaces_.at(id));
        found = ids.emplace(id, replacement).first;
      }
      id = found->second;
    }
    result.geometry_ = Manifold(mesh);
    CheckGeometry(result.geometry_);
    return result;
  }
  const ModelAppearance& Appearance(const MeshGL64& mesh,
                                    size_t triangle) const {
    auto it = std::upper_bound(mesh.runIndex.begin(), mesh.runIndex.end(),
                               uint64_t(3 * triangle));
  ...
  static double Decode(uint8_t c) {
    static const std::array<double, 256> linear = [] {
      ...
      for (size_t i = 0; i < table.size(); ++i) {
        double x = i / 255.0;
        table[i] =
            x <= 0.04045 ? x / 12.92 : std::pow((x + 0.055) / 1.055, 2.4);
      }
      return table;
    }();
    return linear[c];
  ...
  static uint8_t Encode(double x) {
    ...
    return uint8_t(std::lround(
        255 *
        (x <= 0.0031308 ? 12.92 * x : 1.055 * std::pow(x, 1 / 2.4) - 0.055)));
  }
  static vec4 Sample(const MeshGL64& mesh, const ModelAppearance& appearance,
                     size_t face, double u, double v) {
    const double
        *a = &mesh.vertProperties[mesh.triVerts[face * 3] * mesh.numProp],
        *b = &mesh.vertProperties[mesh.triVerts[face * 3 + 1] * mesh.numProp],
        *c = &mesh.vertProperties[mesh.triVerts[face * 3 + 2] * mesh.numProp];
    auto property = [&](int i) {
      return (1 - u - v) * a[i] + u * b[i] + v * c[i];
    };
    vec4 result = appearance.color;
    if (appearance.colorIndex >= 0)
      for (int i = 0; i < 4; ++i)
        result[i] *= property(appearance.colorIndex + i);
    for (const auto& layer : appearance.layers) {
      double x = property(layer.uvIndex), y = property(layer.uvIndex + 1);
      if (layer.repeat) {
        x -= std::floor(x);
        y -= std::floor(y);
      } else if (x < layer.coverage[0] - 1e-9 || x > layer.coverage[2] + 1e-9 ||
                 y < layer.coverage[1] - 1e-9 || y > layer.coverage[3] + 1e-9)
        continue;
      ...
      for (int dy = 0; dy < 2; ++dy)
        for (int dx = 0; dx < 2; ++dx) {
          int px = layer.repeat ? (ix + dx + image.width) % image.width
                                : std::min(ix + dx, image.width - 1);
          int py = layer.repeat ? (iy + dy + image.height) % image.height
                                : std::min(iy + dy, image.height - 1);
          size_t offset = 4 * (size_t(py) * image.width + px);
          double weight = (dx ? fx : 1 - fx) * (dy ? fy : 1 - fy),
                 alpha = image.rgba[offset + 3] / 255.0;
          for (int i = 0; i < 3; ++i)
            foreground[i] += weight * alpha * Decode(image.rgba[offset + i]);
          foreground.w += weight * alpha;
        }
      double alpha = foreground.w * layer.opacity,
             combined = alpha + result.w * (1 - alpha);
      for (int i = 0; i < 3; ++i)
        result[i] = combined == 0 ? 0
                                  : (foreground[i] * layer.opacity +
                                     result[i] * result.w * (1 - alpha)) /
                                        combined;
      result.w = combined;
    }
    return result;
