...
std::vector<int> Manifold::GetHalfedges() const {
  ...
  std::vector<int> ret;
  ret.reserve(3 * halfedges.size());
  for (size_t i = 0; i < halfedges.size(); ++i) {
    ret.push_back(halfedges.Start(i));
    ret.push_back(halfedges.End(i));
    ret.push_back(halfedges.Pair(i));
  }
  return ret;
}
...
std::vector<int> Manifold::GetTriangles() const {
  const Impl& impl = *GetCsgLeafNode().GetImpl();
  std::vector<int> ret;
  ret.reserve(impl.halfedge_.size());
  for (size_t i = 0; i < impl.halfedge_.size(); ++i)
    ret.push_back(impl.halfedge_.Start(i));
  return ret;
}

std::vector<Polygons> Manifold::Slices(double bottomZ, double topZ,
                                       int count) const {
  if (count < 0) throw std::invalid_argument("slice count must be nonnegative");
  std::vector<Polygons> result;
  result.reserve(count);
  for (int i = 0; i < count; ++i)
    result.push_back(Slice(
        count == 1 ? bottomZ : bottomZ + (topZ - bottomZ) * i / (count - 1)));
  return result;
}
